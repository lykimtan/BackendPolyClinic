...
export const getAllFnA = async (req, res) => {
    try {
        const fnas = await FnA.find().populate('askerId', 'firstName lastName').populate('doctorId', 'firstName lastName specialization');
...
export const getFnAByAsker = async (req, res) => {
    try {
        ...
        const fnas = await FnA.find({askerId}).populate('askerId', 'firstName lastName').populate('doctorId', 'firstName lastName specialization');
        res.status(200).json({data: fnas});
    }
    catch(error) {
        res.status(400).json({message: error.message});
...
export const getFnAByResponder = async (req, res) => {
    try {
        ...
        const fnas = await FnA.find({doctorId}).populate('askerId', 'firstName lastName').populate('doctorId', 'firstName lastName specialization');
        res.status(200).json({data: fnas});
    }
    catch(error) {
        res.status(400).json({message: error.message});
...
export const getPublishedFnA = async (req, res) => {
    try {
        const fnas = await FnA.find({isPublished: true}).populate('askerId', 'firstName lastName').populate('doctorId', 'firstName lastName specialization');
        res.status(200).json({data: fnas});
    }
    catch(error) {
        res.status(400).json({message: error.message});
