...
export const handleUploadMedicationImage = upload.single('medicationImage');
...
export const deleteImage = (imagePath) => {
  if (imagePath) {
    try {
      const fullPath = path.join(__dirname, '../../uploads/medications', path.basename(imagePath));
...
export const getImageUrl = (filename) => {
  if (filename) {
    return `/uploads/medications/${filename}`;
  }
  return '';
};